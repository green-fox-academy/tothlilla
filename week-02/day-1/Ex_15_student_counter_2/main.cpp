...
{
  std::vector<std::string> nameMoreThanFourCandies;
  for (int i = 0; i < students.size(); ++i) {
    if (students[i].second.at("candies") > 4){
      nameMoreThanFourCandies.push_back(students[i].first);
    }
  }
  return nameMoreThanFourCandies;
}
...
int sumOfAgeWithLessThanFiveCandies(const std::vector<std::pair<std::string, std::map<std::string, int>>>& students)
{
  ...
  for (int i = 0; i < students.size(); ++i) {
    if (students[i].second.at("candies") < 5){
      sumAges += students[i].second.at("age");
    }
  }
  return sumAges;
